...
local function write()
  ...
  if not ok then
    -- simplify message to match :w output
    vim.notify(result:gsub('^Vim:', ''), vim.log.levels.ERROR)
  end
end
...
local save_or_format = action('Save/Format', function()
  write()
  format()
end, vscode 'workbench.action.files.save')
...
-- this requires the Periscope VSCode extension (JoshMu.periscope)
