# See https://rycee.gitlab.io/home-manager/options.html
{
  ...
  home.activation."dotsman-install" = lib.hm.dag.entryAfter ["writeBoundary"] ''
    ...
    ${userSettings.dotfilesDir}/scripts/dotsman/dotsman.sh install all --yes
  '';

  # Enable shell for Darwin systems (for Linux systems is enabled via a NixOS module)
  heyzec.shell.enable = lib.mkDefault (lib.strings.hasSuffix "darwin" pkgs.stdenv.hostPlatform.system);
  heyzec.neovim.enable = lib.mkDefault (lib.strings.hasSuffix "darwin" pkgs.stdenv.hostPlatform.system);
